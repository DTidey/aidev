...
## Purpose
This repository uses a spec-first, multi-agent workflow. The spec in `docs/specs/<nn>-<slug>.md`
...
4. Tester: add/update tests mapped to acceptance criteria; add at least 3 adversarial cases beyond
   the spec edge cases.
...
- Every new spec/test-plan/PR-draft change packet must use the same two-digit prefix, such as
  `03-my-change.md`.
...
- Understand first: restate the problem, goal, affected area, and expected outcome before writing
  code or a spec. Do not assume silently.
- Surface uncertainty with options: if requirements are unclear, ask; present two or three
  reasonable approaches and recommend the simplest sustainable one.
...
- Code-changing specs must document security considerations or explicitly state no meaningful
  security impact.
...
- Write the minimum code that correctly solves the problem; avoid speculative features and
...
- Require these status checks before merge (set the exact job names from your CI workflow):
  - `<CI test job name>`
  - `<CodeQL job name>` (remove if not using CodeQL)
- Dismiss stale approvals when new commits are pushed.
- Block force pushes and branch deletion on `main`.
- Keep `.github/CODEOWNERS` current so review ownership is explicit.
- Keep Dependabot enabled for your package ecosystems and GitHub Actions updates.
...
- `<security command>` passes for code-changing work.
- `<lint command>` and `<test command>` pass.
- CI is green.
