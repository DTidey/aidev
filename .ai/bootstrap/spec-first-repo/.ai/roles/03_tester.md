...
- Review the spec's `Security considerations` section and ensure security-relevant behavior is
  covered by tests or called out as residual risk.
...
- If the security impact is unclear or inadequately testable, report it explicitly using the
  blocker format rather than assuming "no impact".
